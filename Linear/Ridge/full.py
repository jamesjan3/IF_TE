...
Evaluation strategy: 5-fold CV. The data is divided into 10
chunks of 10% each. Each fold holds out 2 chunks (20%) and trains on the
remaining 8 (80%).

Hyperparameter tuning: RidgeCV with log-spaced alpha grid, tuned via
leave-one-out CV internally on each training fold.
...
ACTIVE_SUBSETS = ["utr5_nt", "cds_nt", "utr3_nt", "lengths", "kozak", "codon_freq", "aa_freq", "utr5_k2", "utr5_k3", "utr5_k4","cds_k2","cds_k3","cds_k4","utr3_k2","utr3_k3","utr3_k4", "min_dg", "cds_wobble_nt", "kozak"]
...
for name, prefix in DYNAMIC_SUBSETS.items():
    feature_subsets[name] = [c for c in df.columns if c.startswith(prefix)]
